      ...
      const batchResults = await Promise.all(
        batch.map(async (file) => {
          try {
            ...
            if (file.type === 'image/jpeg' && pageSize === 'auto' && margin === 0) {
              imageBuffer = buffer;
            } else {
              ...
              if (margin > 0 && margin < 50) {
                ...
                const newWidth = metadata.width ? Math.floor(metadata.width * (1 - marginPercent)) : undefined;
                const newHeight = metadata.height ? Math.floor(metadata.height * (1 - marginPercent)) : undefined;
      ...
      for (const result of batchResults) {
        if (result.success && result.buffer) {
          try {
            ...
            failedImages.push({ 
              name: result.name, 
              error: "Failed to embed in PDF: " + (error instanceof Error ? error.message : "Unknown error")
            });
          }
